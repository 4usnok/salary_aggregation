        ...
        for dictionary in data:
            ...
            if dt_value and value is not None and dt_from_obj <= dt_value <= dt_upto_obj:
                ...
                if group_type == 'hour':
                    ...
                    key = dt_value.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
                elif group_type == 'year':
                    # Группировка по годам (первое число года)
                    key = dt_value.replace(month=1, day=1, hour=0, minute=0, second=0, microsecond=0)
                else:
